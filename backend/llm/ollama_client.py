...
class OllamaClient:
    def __init__(
        self, model: str = DEFAULT_MODEL, host: str = OLLAMA_URL, timeout: int = 120
    ):
        self.model = model
        self.host = host
    ...
    def _verify_connection(self):
        try:
            ...
            if self.model not in available:
                print(
                    f"[Ollama] Warning: '{self.model}' not found locally. Run: ollama pull {self.model}"
                )
    ...
    def chat_stream(self, messages: list):
        ...
        with requests.post(
            f"{self.host}/api/chat", json=payload, stream=True, timeout=(5, None)
        ) as r:
            r.raise_for_status()
