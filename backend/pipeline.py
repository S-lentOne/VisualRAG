...
from rag import EpisodeStore, KnowledgeBase, Retriever, build_query_from_detections
...
DEFAULT_CONFIG = {
    ...
    "coco_model_path": "backend/models/yolov8s.pt",
    "confidence": 0.35,
...
class Pipeline:
    ...
    def setup(self):
        ...
        kb = KnowledgeBase(
            db_path=cfg["db_path"], embedder=embedder, chroma_client=chroma_client
        )
        kb.build()
    ...
    def run_image(
        self, path: str, on_result: Optional[Callable] = None
    ) -> PipelineResult:
        self._check_ready()
    ...
    def run_video(self, path: str) -> list:
        ...
        print(
            f"[Video] Pass 1: running YOLO on every {self.config['frame_interval']} frames..."
        )
        ...
        print("[Video] Pass 2: running RAG + LLM on each snapshot...\n")
        ...
        for i, (t, detections, frame) in enumerate(snapshots):
            ...
            print(
                f"  [{i + 1}/{len(snapshots)}] t={t:.1f}s  {labels or ['nothing']}{change_str}"
            )
    ...
    def _run_capture_loop(self, cap, source_label: str, on_result: Optional[Callable]):
        ...
        while cap.isOpened():
            ...
            if cfg["show_cv_window"]:
                display = (
                    self._annotate_frame(frame, last_result) if last_result else frame
                )
    ...
    def _analyze(self, detections: list) -> PipelineResult:
        ...
        assert self.episode_store is not None
        cfg = self.config
        ...
        response = (
            self._stream_response(pkg.messages)
            if cfg["stream"]
            else self.llm.chat(pkg.messages)
        )
